...
class SecurityPolicyTest {
	...
	void testBasicBruteForce() throws IPCreationFailed {
		//Set up mock
		MockDB q = new MockDB() {
			private int [] simulatedFailuresInLastXMins = {23,0};
			private int count = 0;
			@Override
			public int getNumFailedByRequestInLastXMins(LoginRequest signature, int xMins) {
				...
				count++;
				return failures;//force fail login
			}
		};
		...
		/**
		 * Due to the MockDB config, first handleRequest should activate the policy to block request signature
		 */
